...
Save Layout
Delete Layout" | rofi -theme ~/.config/polybar/colorblocks/scripts/rofi/launcher.rasi -i -dmenu -no-custom -p "Select action")
  ...
  LAYOUT_NAME=$(echo "$LAYOUT_NAMES" | rofi -theme ~/.config/polybar/colorblocks/scripts/rofi/launcher.rasi -i -dmenu -p "Select layout (you may type new name when creating)" | sed 's/\s/_/g') # ask for selection
...
SPECIFIC (CHOOSE)
MATCH ANY" | rofi -theme ~/.config/polybar/colorblocks/scripts/rofi/launcher.rasi -i -dmenu -p "How to identify windows? (xprop style)")
  ...
  if [[ "$CRITERION" = "default" ]]; then
    ...
    while true; do
      ...
      if [ ! -z "$LINE_NUM" ]; then
        ...
        SELECTED_OPTION=$(cat -n $LAYOUT_FILE | sed -n "${LINE_NUM},$(expr $LINE_NUM + 2)p" | awk '{$2="";print $0}' | rofi -theme ~/.config/polybar/colorblocks/scripts/rofi/launcher.rasi -i -dmenu -no-custom -p "Choose the matching method for${NAME%,}" | awk '{print $1}')
